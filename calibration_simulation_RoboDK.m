...
Pc0_in_tracker = get_SMR_pos(1);
P_c0_n = T_tracker_ref\[Pc0_in_tracker;1];                                     % represents in reference frame
P_c0_a = P_c0_n;
...
samples = [MeasurePosition(theta_random_vec_deg,1);ones(1,num_of_pts)];
...
while j<1000
    ...
    for i=1:num_of_pts                                                      % repeat num_of_pts times
        ...
        A_tilde(i*3-2:i*3,:) = A_tilde_matrix(twist_matrix_n,eta_matrix,P_n,theta_random_vec(i,:));  % A matrix calculation
    end
    ...
    P_c0_n = [P_c0_n(1:3) + d_eta(37:39);1];
    ...
    if norm(d_eta) < 1e-6                                                   % quit the for loop if deviation is less than 1e-5
        break;
    end
    %% plot
    clf;                                                                    % clear plot
    draw_manipulator_points(twist_matrix_n,P_c0_n,'b');                     % draw nominal axis
    drawnow;
end
%% plot again
...
truth_positions = [MeasurePosition(test_angles_deg,1);ones(1,num_of_test_points)];           % launch sim on RoboDK for data
