...
dPc = zeros(3*num_of_pts,1);                        % deviation of position
...
%         norm(dpc)                                                           % calculate norm of dpc
    ...
    B = B_matrix(twist_matrix_n,2,3);                       % amazing matrix
    k = (A*B)\dPc;
    norm(k)
    ...
    observibility_index = [observibility_index singular_value_mul^(1/rows)/sqrt(num_of_pts)];
    %% composition
    for i=1:6
